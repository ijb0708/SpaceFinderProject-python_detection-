...
print(sqrt(FRAME_WIDTH**2  + FRAME_WIDTH**2)/12, " ", sqrt(FRAME_WIDTH**2  + FRAME_WIDTH**2)/2)
...
while(True) :
    ...
    Rects, RectCount = detectionRect(processing, filterSize = (90, 300), epsilonDeg=0.08, Type = cv2.RETR_TREE)
    ...
    if k == ord("c") or k == ord("C") :
        ...
        for idx, rect in enumerate(Rects) :
            space = SpaceClass(idNum=idx, realLocations=rect.reshape(-1, 2))
            spaces.append(space)
    ...
    if k == ord("d") or k == ord("D") :
        ...
        for space in spaces :
            ...
            for f in range(0, MAX_FLOOR) :
                if pointY> ((FRAME_HEIGHT / MAX_FLOOR) * f) and ((FRAME_HEIGHT / MAX_FLOOR+1) * (f+1)) >= pointY :
        ...
        with open(PROJECT_FILE_PATH + "space_jsons/drawSpaceData"+ PAKINGLOT +".json", 'w') as outFile:
    ...
    if k == ord("m") or k == ord("M") :
        ...
        with open(PROJECT_FILE_PATH + "space_jsons/realSpaceData"+ PAKINGLOT +".json", 'w') as outFile:
            json.dump(data, outFile, indent=4)        
