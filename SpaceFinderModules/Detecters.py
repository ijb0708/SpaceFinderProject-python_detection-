...
def detectionRect(img, epsilonDeg=0.1, filterSize = (100, 1000), Type = cv2.RETR_EXTERNAL):
    ...
    _, contours, hierarchy = cv2.findContours(img, Type, cv2.CHAIN_APPROX_TC89_KCOS)
    ...
    for idx, cnt in enumerate(contours):
        ...
        if(size == 4 and isTree):
            if(checkRectSize(approx, filterSize[0], filterSize[1]) and checkRectAngle(approx, 350, 370)):
                Rects.append(approx)
                RectCount+=1
