...
from SpaceFinderModules.ImageConverters import ToEdgeByAccuratey,\
    ToEdgeByObjectly
from SpaceFinderModules.SpaceFinderClasses import SpaceClass
import cx_Oracle as ora
import numpy as np


PROJECT_FILE_PATH="C:/Users/Administrator/Desktop/주차장_프로젝트/SpaceFinder_develop/"
PAKINGLOT = '41'
...
def maskRoi(gray_img, approx, firstLoc):
    approx = np.array([[approx[0]-firstLoc], [approx[1]-firstLoc], [approx[2]-firstLoc], [approx[3]-firstLoc]])
...
def readData():
    ...
    with open(PROJECT_FILE_PATH + "space_jsons/realSpaceData"+ PAKINGLOT +".json", 'r') as inFile:
        json_data = json.load(inFile)
             
        for json_space in json_data['spaces'] :
            print(json_space['realSpace'], ' type : ', type(json_space['realSpace']))
            space = SpaceClass(idNum=json_space['id'], realLocations=json_space['realSpace'])
...
#         print(space.getRealLocations(), type(space.getRealLocations()))
        ...
        cv2.rectangle(frame, (b_rect[0], b_rect[1]), (b_rect[0] + b_rect[2], b_rect[1] + b_rect[3]), (255, 0, 0), 3)
        roi = process_image[b_rect[1]:b_rect[1]+b_rect[3], b_rect[0]:b_rect[0]+b_rect[2]]
